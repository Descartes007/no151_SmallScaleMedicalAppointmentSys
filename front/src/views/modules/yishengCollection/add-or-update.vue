    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="huanzheId" name="huanzheId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="yishengCollectionTypes">
                        <el-select v-model="ruleForm.yishengCollectionTypes" :disabled="ro.yishengCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in yishengCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="yishengCollectionValue">
                        <el-input v-model="ruleForm.yishengCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    yishengId: false,
                    huanzheId: false,
                    yishengCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    yishengId: '',
                    huanzheId: '',
                    yishengCollectionTypes: '',
                    insertTime: '',
                },
                yishengCollectionTypesOptions : [],
                huanzheOptions : [],
                yishengOptions : [],
                rules: {
                   yishengId: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=yisheng_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.yishengCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`yishengCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.yishengCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
